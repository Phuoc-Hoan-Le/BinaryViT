...
class ViTSelfAttention(nn.Module):
    def __init__(self, config: ViTConfig, layer_num) -> None:
        ...
        self.query = QuantizeLinear(config.hidden_size, self.all_head_size, bias=config.qkv_bias, config=config)
        self.key = QuantizeLinear(config.hidden_size, self.all_head_size, bias=config.qkv_bias, config=config)
        self.value = QuantizeLinear(config.hidden_size, self.all_head_size, bias=config.qkv_bias, config=config)
