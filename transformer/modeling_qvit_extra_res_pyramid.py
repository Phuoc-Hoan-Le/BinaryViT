...
from .utils_quant import QuantizeLinear, QuantizeConv2d, BinaryQuantizer, ZMeanBinaryQuantizer, BiTBinaryQuantizer, BiTHeadBinaryQuantizer
...
class ViTSelfAttention(nn.Module):
    def __init__(self, config: ViTConfig, layer_num) -> None:
        ...
        self.query = QuantizeLinear(config.hidden_size[config.stages[layer_num]], self.all_head_size, bias=config.qkv_bias, config=config)
        self.key = QuantizeLinear(config.hidden_size[config.stages[layer_num]], self.all_head_size, bias=config.qkv_bias, config=config)
        self.value = QuantizeLinear(config.hidden_size[config.stages[layer_num]], self.all_head_size, bias=config.qkv_bias, config=config)
        ...
        if self.reduction_ratio > 1:
            ...
            self.reduction = QuantizeLinear(config.hidden_size[config.stages[layer_num]], self.all_head_size, bias=config.qkv_bias, config=config)
